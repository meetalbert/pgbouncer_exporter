...
var (
	metricMaps = map[string]map[string]ColumnMapping{
		...
		"stats_averages": {
			"database":                {LABEL, "N/A", 1, "N/A"},
			"xact_count":              {GAUGE, "sql_transactions_per_second", 1, "Average transactions per second"},
			"query_count":             {GAUGE, "queries_per_second", 1, "Average queries per second"},
			"bytes_received":          {GAUGE, "received_bytes_per_second", 1, "Average received bytes per second"},
			"bytes_sent":              {GAUGE, "sent_bytes_per_second", 1, "Average sent bytes per second"},
			"xact_time":               {GAUGE, "server_in_transaction_seconds_per_second", 1e-6, "Average seconds spent in transaction per second"},
			"query_time":              {GAUGE, "queries_duration_seconds_per_second", 1e-6, "Average seconds spent executing queries per second"},
			"wait_time":               {GAUGE, "client_wait_seconds_per_second", 1e-6, "Average seconds spent waiting for a server per second"},
			"server_assignment_count": {GAUGE, "server_assignments_per_second", 1, "Average server assignments per second"},
			"client_parse_count":      {GAUGE, "client_parses_per_second", 1, "Average client Parse messages per second"},
			"server_parse_count":      {GAUGE, "server_parses_per_second", 1, "Average server Parse messages per second"},
			"bind_count":              {GAUGE, "binds_per_second", 1, "Average Bind messages per second"},
		},
		"pools": {
			"database":              {LABEL, "N/A", 1, "N/A"},
